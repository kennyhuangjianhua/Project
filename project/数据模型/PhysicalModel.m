...
+ (PhysicalModel *)getPhysicalModelFromDBWithUserID:(NSString *)userID
{
    NSString *where = [NSString stringWithFormat:@"userId = '%@'", userID];
    PhysicalModel *model = [PhysicalModel searchSingleWithWhere:where orderBy:nil];
    if (!model) {
        model = [PhysicalModel simpleInitWithUserID:userID];
        [model saveToDB];
    }
...
+ (PhysicalModel *)simpleInitWithUserID:(NSString *)userID
{
...
- (NSInteger)duration
{
    if (_duration != 30 && _duration != 60 && _duration != 90) {
        _duration = 30;
    }
